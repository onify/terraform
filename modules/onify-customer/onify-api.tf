...
resource "kubernetes_ingress_v1" "onify-hub-api" {
  ...
  spec {
    tls {
      hosts = ["${local.client_code}-${local.onify_instance}-hub-api.${var.external-dns-domain}"]
    ...
    dynamic "tls" {
      ...
      content {
        hosts = ["${tls.value}-hub-api.${var.external-dns-domain}"]
        secret_name = "tls-secret-hub-api-${var.tls}-custom-${tls.value}"
      }
    }
    ingress_class_name = "nginx"
    rule {
      host = "${local.client_code}-${local.onify_instance}-hub-api.${var.external-dns-domain}"
      http {
        path {
          backend {
            service {
    ...
    dynamic "rule" {
      for_each = var.custom_hostname != null ? toset(var.custom_hostname) : []
      content {
        host = "${rule.value}-hub-api.${var.external-dns-domain}"
        http {
          path {
            backend {
              service {
